...
import { getAPIKey, getSecondaryKey } from '../utils/polotno-keys';

const saveAsVideo = async ({ store, pixelRatio, fps, onProgress }) => {
  const json = store.toJSON();
  const req = await fetch(
    `https://api.polotno.dev/api/renders?KEY=${getAPIKey()}`,
    {
      method: 'POST',
      headers: {
  ...
  while (true) {
    const jobReq = await fetch(
      `https://api.polotno.dev/api/renders/${job.id}?KEY=${getAPIKey()}`
    );
