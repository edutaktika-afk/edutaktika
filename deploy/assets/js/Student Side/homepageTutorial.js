/**
 * Homepage Tutorial System for Students
 * Similar to the Teacher tutorial, but for student homepage
 */
...
const STUDENT_HOMEPAGE_TUTORIAL_STEPS = [
  {
    id: 'welcome',
    title: 'Welcome to Edutaktika! 🎓',
    content: 'This interactive tutorial will guide you through the main features of the Edutaktika platform. You\'ll learn how to access lessons, view leaderboards, and manage your profile!',
    position: 'center',
    showNext: true,
    showSkip: true
  },
  {
    ...
    content: 'This is your homepage where you can see the platform overview, mission, vision, and benefits. Scroll down to explore more about Edutaktika and what it offers!',
    position: 'center',
    gif: getGifPath('HOMEPAGE STUDENT.gif'),
    showNext: true,
    showSkip: true
  },
  {
    id: 'subjects',
    title: 'Access Subjects 📚',
    content: 'Click on "Subject" in the top navigation to access your lessons. You can choose from Math, English, or Science. Each subject contains interactive lessons designed for your grade level!',
    position: 'top',
    target: 'nav ul li.dropdown:first-of-type a.dropbtn',
    gif: getGifPath('SUBJECTS STUDENTS.gif'),
    showNext: true,
    showSkip: true
  },
  {
    ...
    content: 'Once you select a subject, you\'ll see all available lessons for your grade level. Click "View" to open a lesson in presentation mode. You can navigate between pages and control animations!',
    position: 'top',
    target: 'nav ul li.dropdown:first-of-type a.dropbtn',
    gif: getGifPath('OPENING LESSONS.gif'),
    showNext: true,
    showSkip: true
  },
  {
    id: 'leaderboards',
    title: 'Leaderboards 🏆',
    content: 'Check the "Leaderboard" dropdown to see student rankings by subject. View top performers in Math, English, and Science. See where you rank and get motivated to improve!',
    position: 'top',
    target: 'nav ul li.dropdown:last-of-type a.dropbtn',
    gif: getGifPath('LEADERBOARDS STUDENT.gif'),
    showNext: true,
    showSkip: true
  },
  {
    ...
    content: 'Click on "Profile" to view your account information, see your grade level and section, and access the logout function. Your profile shows all your important student details!',
    position: 'top',
    target: '#profileBtn',
    gif: getGifPath('PROFILE VIEW LOGOUT STUDENT.gif'),
    showNext: true,
    showSkip: true
  },
  {
    ...
    content: 'You\'re all set to use Edutaktika! Remember, you can always access this tutorial again by clicking the "Tutorial" button in the navigation. Happy learning!',
    position: 'center',
    showNext: false,
    showSkip: false,
    showFinish: true
...
class StudentHomepageTutorial {
  ...
  init() {
    // Only show tutorial button on student homepage
    const pathname = window.location.pathname.toLowerCase();
    const isHomepage = pathname.includes('homepage.html') || 
                       pathname.endsWith('/') ||
                       pathname.endsWith('/student/') ||
                       (pathname.includes('/student/') && !pathname.includes('.html') && pathname.split('/').pop() === '');
  ...
  createTutorialButton() {
    ...
    const tryAddButton = () => {
      const button = document.getElementById('student-homepage-tutorial-btn');
      ...
      tutorialListItem.id = 'student-homepage-tutorial-li';
      
      const newButton = document.createElement('a');
      newButton.id = 'student-homepage-tutorial-btn';
      newButton.href = '#';
      newButton.innerHTML = '<i class="fas fa-question-circle"></i> Tutorial';
      newButton.className = 'student-homepage-tutorial-trigger';
      newButton.onclick = (e) => {
        e.preventDefault();
        this.startTutorial();
      };
  ...
  showStep() {
    const step = STUDENT_HOMEPAGE_TUTORIAL_STEPS[this.currentStep];
    if (!step) return;
  ...
  createOverlay(step) {
    ...
    const existing = document.getElementById('student-homepage-tutorial-overlay');
    if (existing) existing.remove();

    const overlay = document.createElement('div');
    overlay.id = 'student-homepage-tutorial-overlay';
    ...
    const content = `
      <div class="tutorial-modal-content">
        <div class="tutorial-header">
          <h3>${step.title}</h3>
          <button class="tutorial-close" onclick="studentHomepageTutorial.closeTutorial()">&times;</button>
        </div>
        ...
        <div class="tutorial-footer">
          <span class="tutorial-step-count">Step ${this.currentStep + 1} of ${STUDENT_HOMEPAGE_TUTORIAL_STEPS.length}</span>
          <div class="tutorial-buttons">
            ${step.showSkip ? `<button class="tutorial-btn tutorial-btn-skip" onclick="studentHomepageTutorial.skipTutorial()">Skip Tutorial</button>` : ''}
            ${step.showNext ? `<button class="tutorial-btn tutorial-btn-next" onclick="studentHomepageTutorial.nextStep()">Next <i class="fas fa-arrow-right"></i></button>` : ''}
            ${step.showFinish ? `<button class="tutorial-btn tutorial-btn-finish" onclick="studentHomepageTutorial.finishTutorial()">Finish <i class="fas fa-check"></i></button>` : ''}
          </div>
        </div>
      </div>
    `;
  ...
  nextStep() {
    if (this.currentStep < STUDENT_HOMEPAGE_TUTORIAL_STEPS.length - 1) {
      this.currentStep++;
      this.showStep();
    }
  }
  ...
  skipTutorial() {
    this.closeTutorial();
    localStorage.setItem('edutaktika-student-homepage-tutorial-completed', 'true');
  ...
  closeTutorial() {
    this.cleanup();
    this.isOpen = false;
    const overlay = document.getElementById('student-homepage-tutorial-overlay');
  ...
  injectStyles() {
    ...
    style.textContent = `
      ...
      #student-homepage-tutorial-overlay {
        position: fixed;
        top: 0;
        left: 0;
        right: 0;
      ...
      .student-homepage-tutorial-trigger {
        display: flex;
        align-items: center;
        height: 48px;
        line-height: 48px;
      ...
      .student-homepage-tutorial-trigger:hover {
        color: #137cbd;
        background: rgba(19, 124, 189, 0.1);
        border-radius: 4px;
      }
      
      .student-homepage-tutorial-trigger i {
        font-size: 16px;
      }
      
      #student-homepage-tutorial-li {
        list-style: none;
        margin: 0 10px;
        position: relative;
        height: 100%;
...
let studentHomepageTutorial;
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    studentHomepageTutorial = new StudentHomepageTutorial();
    window.studentHomepageTutorial = studentHomepageTutorial; // Make it globally accessible
  });
} else {
  studentHomepageTutorial = new StudentHomepageTutorial();
  window.studentHomepageTutorial = studentHomepageTutorial;
}
